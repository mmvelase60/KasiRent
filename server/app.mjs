...
export function app(db) {
 ...
 api.post('/payments',async(req,res)=>{const p=z.object({id:z.string().uuid(),tenancy_id:text,amount:cents,method:z.enum(['Cash','EFT','Bank deposit']),paid_on:date,reference:z.string().trim().max(200)}).parse(req.body);if(!(await rows('SELECT id FROM tenancies WHERE id=$1 AND owner=$2',[p.tenancy_id,req.owner])).length)return res.status(404).json({error:'Tenant not found.'});const existing=(await rows('SELECT * FROM payments WHERE id=$1',[p.id]))[0];if(existing){if(existing.owner!==req.owner || existing.tenancy_id!==p.tenancy_id || existing.amount!==p.amount || existing.method!==p.method || (existing.paid_on instanceof Date ? existing.paid_on.toISOString().slice(0,10) : String(existing.paid_on).slice(0,10))!==p.paid_on || existing.reference!==p.reference)return res.status(409).json({error:'Payment reference already used. Refresh before retrying.'});return res.json(existing);}res.json((await rows('INSERT INTO payments (id,owner,tenancy_id,amount,method,paid_on,reference) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *',[p.id,req.owner,p.tenancy_id,p.amount,p.method,p.paid_on,p.reference]))[0]);});
